...
def main():
    ...
    # 收件曲线(单季值)：判断洪峰相对你 PD 的位置(峰在你身后 = 偏有利)
    if rec:
        ann = sum(v for _, v in rec[-4:])   # 近 4 季 = 年化率
        pk_tag = max(rec, key=lambda x: x[1])[0]
        p(f"\n收件(单季,EB1 A+B+C)近4季年化≈{ann:,}/年; 峰值在 {pk_tag}。"
          " 收件为单季值;流入缩=身后堆积变小。f 形状以洪峰相对你 PD 的位置微调(峰在身后→f 偏中下)。")
    ...
    p(f"\n## demandWall 建议(顾问性)")
    p(f"f 诚实区间 ≈ [{f_lo:.2f}, {f_hi:.2f}] → 建议 中心≈{sug_center:.2f}、σ≈{sug_sigma:.2f}")
