# EB1A Predictor V22 - Claude Code Bootstrap (Updated with Data Sources)

## 上下文

V11-V21 是单文件 HTML 预测工具,演进路径见 README。所有版本都拟合 cutoff 时间序列,**根本错误**:
1. 把 cutoff 当主状态,而 visa supply 才是
...
完全重写为 agent-based simulation,模拟 DOS 真实月度签证分配。
...
**社区数据整理**:
- GreenCardClock: FY2025 China EB 完整拆分
- Lucid blog (blog.lucidtext.com): EB-5 月度精确解读
- ImmigrationRoad: CP/AOS 比例计算 (~85%/15%)
- visabulletin.ai: 1995-2026 时间线 + cohort comparison
...
│   └── policy.py
├── tests/
├── versions/  (V11-V21 HTML)
├── webapp/    (FastAPI + 新前端)
└── index.html (current V21)
```
...
- PD: 2024 年中, EB-1A China
- 当前 cutoff (June 2026): 2023-04-01 (FAD), 2023-12-01 (Filing)
- 首次运行时要求用户输入自己的 PD、类别、出生国等信息

V18-V21 共识 P50 (以 2024 年中 PD 为例): 2028 末 ~ 2029 中

V22 目标: 基于真实 cohort 数据 + INA 规则,给出可解释 + 可回测的 P50。
